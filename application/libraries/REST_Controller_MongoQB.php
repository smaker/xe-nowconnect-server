...
abstract class REST_Controller extends CI_Controller
{
	...
	*/
	...
	public function __construct()
	{
		...
		if ((config_item('rest_enable_keys') OR config_item('rest_enable_logging')))
		{
	...
	protected function _detect_api_key()
	{
		...
		if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name)))
		{
			...
			$columnList = array(config_item('rest_key_column'), 'user_id', 'level', 'ignore_limits', 'api_status', 'is_private_key', 'ip_addresses');
			if ( ! ($row = $this->rest->db->rest->{config_item('rest_keys_table')}->findOne($query, $columnList)))
			{
				return FALSE;
			}
			...
			isset($row['api_status']) AND $this->rest->api_status = $row['api_status'];

			if($row['api_status'] != 'accepted')
			{
				return FALSE;
			}
	...
	{
		return $this->rest->db->rest->{config_item('rest_logs_table')}->insert(array(
	...
	protected function _check_limit($controller_method)
	{
		...
		$query = array('uri' => $this->uri->uri_string(), 'api_key' => $this->rest->key);
		$columnList = array('hour_started', 'count');
		$result = $this->rest->db->rest->{config_item('rest_limits_table')}
				->findOne($query, $columnList);

		// No calls yet, or been an hour since they called
		if ( ! $result OR $result['hour_started'] < time() - (60 * 60))
		{
			// Right, set one up from scratch
			$this->rest->db->rest->{config_item('rest_limits_table')}->insert(array(
				'uri' => $this->uri->uri_string(),
				'api_key' => isset($this->rest->key) ? $this->rest->key : '',
				'count' => 1,
				'hour_started' => time(),
			...
			$this->rest->db->rest->{config_item('rest_limits_table')}->ensureIndex('date', array('expireAfterSeconds' => 60 * 60 * 24 ));
		}
		...
		else
		{
			...
			$this->rest->db->rest->{config_item('rest_limits_table')}->update(array('uri' => $this->uri->uri_string(), 'api_key' => $this->rest->key), array('$inc' => array('count' => 1) ));
		}
